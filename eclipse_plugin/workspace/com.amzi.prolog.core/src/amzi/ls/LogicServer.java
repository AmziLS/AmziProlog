...
public class LogicServer
{
   ...
   public native boolean UnifyFloatParm(int iarg, double f) throws LSException;
	...
	static
	{
		...
		if (macos) userdir = userdir + sl + ".." + sl + ".." + sl + "..";
		String amzijni;
		String amzijnipath;
		if (winos) amzijni = "amzijni.dll";
		else if (macos) amzijni = "libamzijni.jnilib";
		else amzijni = "libamzijni.so";
		String amzidir;
		try {
			// look here first
			System.load(userdir + sl + amzijni);
			//MessageDialog.openError(null, "Found it","here with load");
		}
			...
			try {
				...
				try {
					// ok, lets look in amzidir if it's there
					amzidir = System.getenv("AMZI_DIR");
					if (winos)
						amzijnipath = amzidir + sl + "bin" + sl + amzijni;
					else
						amzijnipath = amzidir + sl + "lib" + sl + amzijni;
					System.load(amzijnipath);
					//MessageDialog.openError(null, "Found it",
					//		"Found using AMZI_DIR: " + amzijnipath);

				}
				catch(UnsatisfiedLinkError ex3) {
					try {
						// ok, maybe an amzi app running parallel to the amzi/apls directory
						amzidir = userdir + sl + ".." + sl + "apls" + sl;
						if (winos)
							amzijnipath = amzidir + "bin" + sl + amzijni;
						else
							amzijnipath = amzidir + "lib" + sl + amzijni;
						//MessageDialog.openError(null, "Looking for JNI",
						//		"load: " + amzijnipath);					
						System.load(amzijnipath);
						//MessageDialog.openError(null, "Found it",
						//		"Found it relatively: " + amzidir);
					}					
					catch(UnsatisfiedLinkError ex4) {
						MessageDialog.openError(null, "Amzi! Initialization Error",
							"Unable to open Amzi! JNI library, " + amzijni);
						System.err.println("Unable to load amzijni library: " + ex4.getMessage());
					}
