...
%
%  This implementation was done with Amzi! Prolog, also available
%  from Amzi! inc.
%
...
banner:-
  ...
  write($Amzi! inc.$),nl,
  write($www.amzi.com$),nl,
  write($info@amzi.com$),nl,nl.
...
% This is a bench mark interface that just solves a bunch
% of cubes and reports on the time, with I/O turned off.

bench:-
  cntr_set(0,1),
  set_prolog_flag(decimal_places, 3),
  START is cputime,
  set_flag(stepmode,off),
  set_flag(output,off),
  repeat,
  T1 is cputime,
  init_solve(random),              % initialize all the stuff
  stages,
  T2 is cputime - T1,
  TT is cputime - START,
  cntr_inc(0,N),
...
% This is a special start point for use in the
% performance probe, just runs once.
...
add_criteria(Piece,Crit):-
  clause(crit(OldCrit),_),
  pieces(OldCrit, OldCritP),
  clause(ghoul(Ghoul),_),
  pieces(Ghoul, GhoulP),
