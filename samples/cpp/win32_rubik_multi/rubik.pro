...
% For DOS programs, load the display predicates that
% take advantage of the Cogent screen manipulation
% predicates.  Otherwise, just load the boring text
% mode version.  Note that these are only loaded when
% the program is initiated with the main/0 predicate.
% The graphical Windows version has its own front
% end and display functions.

load_disp(dos) :- load(rubioscr).
load_disp(X) :- load(rubiotxt).
...
% Called from the Windows GUI version to initialize things.

wininit :-
%  writelog('in wininit'),
%  buginit('c:/temp/bug.log'),
  init_color,
  set_flag(step_mode, off),
  pristine(S),
  cube_print(S).
...
banner:-
  ...
  write($Building Expert Systems in Prolog by Dennis Merritt$),nl,
  write($implemented in Amzi! Prolog$),nl,nl,
  write($For more information see:$),nl,
  write($www.amzi.com$),nl,nl.
...
% run.  It always starts from the same seed, but in this case time/3
% is used to create more random starts.
