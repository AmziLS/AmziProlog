%------------------------------------------------------------
% EQ.PRO - Equation Drawing Code
%
% A sample program illustrating how to use the Amzi! Prolog
% Logic Server and Delphi to display equations as they
% might appear in a text book.
%------------------------------------------------------------
...
% paint_equation is called from Delphi in response to an
...
grapheq(_, X, _, _, _, _) :-
  var(X),
  !,
  w_message($Input error: Use single quotes around upper case elements$),
  fail.
...
unary_op(Op, Prec) :-
  current_op(Prec, A, Op),
  (A == fy ; A == yf ; A== xf ; A == fx).

binary_op(Op, Prec) :-
  current_op(Prec, A, Op),
  not((A == fy ; A == yf ; A== xf ; A == fx)).
...
% bug/1 is a useful print statement for debugging,
% using the extended predicate w_message to display
